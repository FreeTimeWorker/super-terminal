    ...
    public class UseHttpParamterMiddleware
    {
        ...
        public Task InvokeAsync(HttpContext context)
        {
            if (context.Request.ContentType == "application/json")
            {
                ...
                using (var reader = new StreamReader(context.Request.Body, encoding: Encoding.UTF8, detectEncodingFromByteOrderMarks: false, bufferSize: 1024 * 20, leaveOpen: true))
                {
                    var body = reader.ReadToEndAsync().Result;
                    Utf8JsonReader utf8JsonReader = new Utf8JsonReader(Encoding.UTF8.GetBytes(body));
                    try
                    {
